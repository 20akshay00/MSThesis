...
# ╔═╡ 691ff340-5b7b-11ed-1c63-c5e2482720d3
begin
	using JLD, Plots, LaTeXStrings
	using ImageFiltering, ImageEdgeDetection, Images
	using ImageEdgeDetection: Percentile
	theme(:lime)
end

# ╔═╡ d9bd786a-c5c6-46ce-98c4-29f8f47aec9d
...
# ╔═╡ 9469c2a1-ded7-4f9a-8f02-fbd92f675037
begin
	d = load("./data/tri/V=0.16_n=100.jld")
...
# ╔═╡ 44eba1e0-df0d-4799-8fb7-644376079e7e
begin
	canny(σ) = Canny(spatial_scale=σ, high=Percentile(80), low=Percentile(20))
end

# ╔═╡ 3a1d23be-891e-4fd6-b364-2e685eb89b16
begin
	...
	function phase_diagram(t, mu, order_param, err = 1e-2, col_type = "classic", loc = nothing)
	    p = []
	    n = last(size(order_param)) ÷ 2
		...
		c = (col_type == "classic") ? palette([:black, "#02ff00", "#d4d0c8", "#ff0000", :blue]) : palette(["#fbe59b", "#75a5ce", "#e86572", "#7d62a9", :transparent])
	    ...
	    if !isnothing(loc)
	        annotate!(loc..., text("zV/U = $(0.6)", :black, :left, 20))
	    end
...
# ╔═╡ 4aa680e8-5208-4599-b553-43da2c3e9d22
begin
	image = classify(order_param, 0.5e-3)
	blur = imfilter(Gray.(image ./ 3), Kernel.gaussian(0))
	res = convert(Array{Int64}, detect_edges(blur, canny(3.01)))
end;

# ╔═╡ f0a5574b-342c-46c3-b930-5e9bab0d9b57
begin
	p = phase_diagram(t, mu, sort(order_param, dims = 3), 0.5e-3, "modern", [0.03, 1.1])
	plot!(p[4], t, mu, res, st = :heatmap, c = [:transparent, :blue])
end
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "3bbf91d89e4539c852af0d78423204bbdf056d62"
