...
# ╔═╡ b9f569c0-5b90-11ed-1128-1760f1015770
begin
	using JLD, Plots, LaTeXStrings
	using Combinatorics
end

# ╔═╡ 6d950cc0-4570-4ed8-a245-fe21ce977263
using KrylovKit

# ╔═╡ 65419278-62a2-4ca1-962e-088ce26d420b
...
# ╔═╡ bd3d5f8c-5691-4d5c-9478-ed6e073c37f8
ED = ingredients("./ED.jl")

# ╔═╡ eec2e2d5-4d8b-48b5-a923-1345a7ce70d5
begin
	d = load("./data/ed/V=0.14_n=100.jld")
...
# ╔═╡ 5e68d532-518b-4cc1-b727-7a9a7e8ce759
model = ED.BHM(model_params[1:2]..., false, true)

# ╔═╡ 53fb8c89-f556-4949-8ba7-0f19b1bedcb1
begin
	L = model_params[1]
	D(gs, r) = 1/L * sum(ED.expect(gs, model.N[i] * model.N[mod1(i + r, L)]) for i in 1:(L-1))
end

# ╔═╡ 51f52011-6dab-4b9e-8a6e-242f0e41b3ad
S(gs, k) = 1/L * sum(exp(im * k * r) * D(gs, r) for r in 1:L)

# ╔═╡ f7d2ab22-1110-4a74-8802-474f6abab380
sfactor = S.(gs, [π])

# ╔═╡ e6ad54f0-de24-4604-b0e8-b211b2d21515
res = D.(gs, [1])

# ╔═╡ ef917f6e-c8a9-4353-a2f2-8272899ee414
plot(sfactor .|> abs, st = :heatmap)
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "1a6eaede909861d176fa21ff9a824eace34c40b0"
