...
# ╔═╡ 7b462f80-5d1f-11ed-1d26-f3ae48afe619
using JLD, Plots, LaTeXStrings

# ╔═╡ 7623a499-1fb2-4d1c-a31c-3528927d902f
begin
	Vs = 0.0:0.02:0.26
	ops = [load("MFT_4_$(V).jld")["order_param"] for V in Vs];
	t = load("MFT_4_0.0.jld")["t"]
	mu = load("MFT_4_0.0.jld")["mu"]
end;

# ╔═╡ 3754e6bf-dce9-429e-acee-2d7d811e6c03
...
# ╔═╡ 41d0d045-0177-436e-abdc-df919bac73e0
function phase_diagram(t, mu, V, order_param, err = 1e-2, loc = [0, 0])
    p = []
    n = 2
    ...
    if !isnothing(loc)
        annotate!(loc..., text("zV/U = $(4 * V)", :black, :left, 20))
    end
    
    l = @layout [
        [grid(n, 1)] c{0.5w} [grid(n, 1)]
    ]
    plot(p[1:n]..., phases, p[(n+1):end]..., size = ((n + 2) * 500, 1.5 * 500), layout = l)
end

# ╔═╡ b407f30a-bd67-4931-b8db-a09d599ef1f3
p = [phase_diagram(t, mu, Vs[i], sort(ops[i], dims = 3), 1e-2, [0.055, 2.7]) for i in 1:length(Vs)]

# ╔═╡ 1f09e5d9-3209-4595-bb35-c05d0734d50f
[savefig(p[i], "./MFT_$(i).png") for i in 1:length(Vs)]
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "47eb22d7100c782db387ec8ea612e5a3182554fb"
