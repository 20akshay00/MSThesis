...
# ╔═╡ 5b6f7e80-5a15-11ed-1fd7-c381cfc3eb48
begin
	using Plots, LaTeXStrings, FixedPoint, JLD
	using LinearAlgebra, SparseArrays, KrylovKit
end

# ╔═╡ ecf74b10-9705-47c5-99e1-4e167d726c3d
begin
	...
	struct MeanField <: BoseHubbardModel
	    ...
	    nA :: SparseMatrixCSC{Float64, Int64}
		nB :: SparseMatrixCSC{Float64, Int64}
	    ...
	    function MeanField(n_max) 
			...
			return new(n_max, kron(a, id), kron(id, a), kron(n, id), kron(id, n))
		end
	end
	...
	function get_hamiltonian(model :: MeanField, t, mu, U, V, z, params)
	    ...
	    H = - mu * model.nB / (z * t) + (U/(2 * z * t)) * model.nB * (model.nB - I) + (V/t) * (rhoA * model.nB - 0.5 * rhoB * rhoA * I) - psiA * (model.aB + model.aB') + psiB * psiA * I - mu * model.nA / (z * t) + (U/(2 * z * t)) * model.nA * (model.nA - I) + (V/t) * (rhoB * model.nA - 0.5 * rhoA * rhoB * I) - psiB * (model.aA + model.aA') + psiA * psiB * I
		
	    return H 
	end
	
	function get_order_parameter(model, t, mu, U, V, z, atol = 1e-7, rtol = 1e-5)
	    function f(params)
			psi = eigsolve(get_hamiltonian(model, t, mu, U, V, z, params), 1, :SR)[2][1]
			return abs.(expect.([model.aA, model.aB, model.nA, model.nB], [psi]))
		end
		
		return afps(f, rand(4), tol = 1e-5)[:x]
	end
end

# ╔═╡ 6b2ef7f7-94f7-4fac-93d0-408c2ddbb020
#=╠═╡
function get_phases(params, tol = 1e-2)

    function identify(psiA, psiB, rhoA, rhoB)
        c = 4 # default
		err = tol

        if isapprox(rhoA, rhoB, atol = err) # BHM
			if isapprox(psiA, 0, atol = err) 
				c = 0 # MI
			else
				c = 2 # SF
			end
		else
			if isapprox(psiA, 0, atol = err) # eBHM
				c = 1 # DW
			else 
				c = 3 # SS
			end
...
# ╔═╡ 3ad3ca79-ff14-4926-99ab-157543447f5e
begin
	theme(:lime)
	gr()
	p = []
	...
	phases = heatmap(t, mu, get_phases(order_param, 1e-2), 
	...
	annotate!(0.052, 2.75, text("zV/U = $(z * V)", :black, :left, 20))
...
# ╔═╡ df6331ca-8445-4cfe-978b-a55bb1dd0cfc
save("./data/MFT_$(z)_$(round(V, digits=4)).jld", "order_param", order_param, "t", t, "mu", mu, "V", V)

# ╔═╡ 2955ce69-1c4d-4767-948c-3cf60385323a
begin
	model = MeanField(6)
	z, t, mu, U = 4, 0.2, 1.8, 1.0
	V = 0.15
	
 	get_order_parameter(model, t, mu, U, V, z)
end

# ╔═╡ c8cf0c08-e7f6-4dc2-ab69-18a85221520c
# ╠═╡ disabled = true
#=╠═╡
begin
	model = MeanField(6)
	...
	Threads.@threads for k1 in 1:num_points
	    for k2 in 1:num_points
	        order_param[k2, k1, :] .= get_order_parameter(model, t[k1], mu[k2], 1., V, z)
	    end
	end
end
  ╠═╡ =#
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "52895871393ffba912ff74f7e98d6a311aff05a3"
