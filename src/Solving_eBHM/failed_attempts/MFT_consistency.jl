...
# ╔═╡ 220da0a0-099c-11ed-240c-3defa2e43f61
using Plots, LinearAlgebra, LaTeXStrings

# ╔═╡ 7c39ab6e-77bb-4c20-ae3c-cd8db920b445
begin
	...
	function get_order_parameter(model, t, mu, V, z)
	    ground_state(params) = eigvecs(get_hamiltonian(model, t, mu, V, z, params))[:, 1]
		params_old, params_new = [10, 10], [0, 0]
		
		while(true)
			params_old .= params_new
			psi_gs = ground_state(params_new)
			params_new = [expect(model.a, psi_gs), expect(model.n, psi_gs)]

			if(all(abs.(params_old .- params_new) .< 1e-4)) break; end
		end
	...
	function get_num_particles(model, t, mu, V, z)
	    psi = get_order_parameter(model, t, mu, V, z)
...
# ╔═╡ efc8236e-8e0f-46b2-a023-da04f29d5196
begin
	model = MeanField(10)
	...
	Threads.@threads for k1 in 1:size
		...
		end# num_particles[k2, k1] = abs.(get_num_particles(model, t[k1], mu[k2], V, z))
	end
end

# ╔═╡ 448f6d71-8aa5-43c6-8b93-ea77ee178cf4
eigvecs(get_hamiltonian(model, 0.01, 0.4, 1, 2, [0, 0]))[:, 1]

# ╔═╡ 44f833c6-6759-4156-82d4-50d4bedc05f4
begin
	theme(:lime)
	p1 = heatmap(t, mu, order_param[:, :, 1])
	plot!(
