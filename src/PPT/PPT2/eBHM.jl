...
begin
	...
	function get_order_parameter(model, t, mu, U, V, z, init = nothing, depth = 1)
	    ...
	    params_new = isnothing(init) ? rand(0:1000, 4) : init
...
begin
	...
	gr(dpi = 500)
	p = []
	p_names = [L"\Psi_A  = \langle a_A \rangle", L"\Psi_B = \langle a_B \rangle", L"\rho_A = \langle n_A \rangle", L"\rho_B = \langle n_B \rangle"]
	
	cols = palette([:black, "#02ff00", "#d4d0c8", "#ff0000", :blue])
	phase_labels = ["Mott Insulator (MI)", "Density Wave (DW)", "Superfluid (SF)", "Supersolid (SS)", "Unclassified"]
	...
	phases = heatmap(t, mu, get_phases(order_param), 
		c = cols, 
		title = "\neBHM MFT - Phase Diagram",
	    ylabel = "μ/U (chemical potential)",
	    xlabel = "t/U (hopping parameter)",
	    colorbar = false)	
	plot!(
		xlim = [minimum(t), maximum(t)], 
		ylim = [minimum(mu), maximum(mu)],
		legend = true)

	for (c, lab) in zip(cols, phase_labels)
		scatter!(t[1:1], mu[1:1], c = c, label = lab, ms = 1)
	end

	annotate!(0.015, 0.2, text(L"DW-1/2", :black, :right, 15))
	annotate!(0.015, 1.6, text(L"DW-3/2", :black, :right, 15))
	annotate!(0.01, 0.9, text(L"MI-1", :white, :right, 15))
	annotate!(0.01, 2.3, text(L"MI-2", :white, :right, 15))

	annotate!(0.047, 2.0, text(L"V = 0.15U", :black, :right, 25))

	l = @layout [
	    [grid(2, 1)] c{0.5w} [grid(2, 1)]
	]
	plot(p[1], p[2], phases, p[3], p[4], size = (1700, 600), layout = l, margin = 7Plots.mm)

	savefig("eBHM.png")
end

# ╔═╡ 772ce008-a43c-4e75-9556-a0920afc6edf
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "93476e566ca03126d793e48169567696d356ef71"
