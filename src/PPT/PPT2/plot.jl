...
# ╔═╡ 94c96b62-3c28-11ed-2c52-13e9a156f90f
using Plots, PrettyTables, DataFrames, LaTeXStrings

# ╔═╡ 7c20951f-3111-4285-a131-b5893b7c9246
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ 32422f78-3c34-406f-89d0-faf9e906e399
hilbert_dim(N, M) = binomial(N + M - 1, M - 1)

# ╔═╡ eb0f37e1-d721-40ec-88f0-c6652e4c76d6
# begin
# 	ks = 1:23
# 	ns = 1:23
# 	data = hilbert_dim.(ks' .* ones(Int64, length(ns)), ones(Int64, length(ks))' .* ns)
# 	data .= floor.(Int, log10.(data))
# end;

# ╔═╡ 5871f055-40f0-4a80-9482-77433c9e92b2
df = DataFrame(data);

# ╔═╡ f137e69f-e5b3-4252-828a-6d6764da4340
begin
	rename!(df, [Symbol("N = $i") for i in ns])
end

# ╔═╡ d6a364dc-ba1f-4c76-94ff-b66c97217fa7
begin
	gr()
	theme(:dao)
	nlist = 5:30
	mlist = 1:25
	cols = cgrad(:roma, length(nlist), categorical = true, scale = :log10)
	
	p = plot(xlabel = "M (number of lattice sites)", ylabel = L"\log_{10}" * "(Dim(H))", yscale = :log10, xticks=(mlist, mlist), yticks=(10 .^ (1:15), 1:15), title = "\nDimensionality of the Hilbert Space", topmargin = 4Plots.mm, dpi = 500)
	for (i, n) in enumerate(nlist)
		plot!(mlist, hilbert_dim.(n, mlist), lw = 2, lab = "N = $n", c = cols[i])
	end

	plot!(legend = :outerright, framestyle = :box)
	plot!([1, 25], [10^5, 10^5], label = "", c=:black)
	plot!(mlist, ones(length(mlist)), fillrange = 10^5 * ones(length(mlist)), fillalpha = 0.2, c = :black, label = "")
	p
end

# ╔═╡ 52e688f2-9077-485d-aecd-82695dd3d26c
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "0787fc0e07a3d832c37b929a70427b5d4106c81c"
