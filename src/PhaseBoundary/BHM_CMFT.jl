...
begin
	...
	function isMI(model; t, mu, U, psi_t = 1e-6)
		function f(psi)
			gs = eigsolve(-t .* model.Hk .+ U .* model.Hu .- mu .* model.Hn + t .* (-psi .* model.Hmf + 2 * psi^2 * I), 1, :SR)[2][1]

			return expect(gs, model.A[1])
		end
...
# ╔═╡ 289384eb-a762-4aeb-9932-843a5c02fc0b
begin
	struct BHM
	    L :: Int64
	    n_max :: Int64
...
# ╔═╡ 46ca0dd5-0312-472a-8750-644541057ffb
begin 
	npoints = 100
	mu = range(start = 0., stop = 3., length = npoints)
	num_sites = 1:6
	t = zeros((length(num_sites), npoints))
	
	for n in num_sites
		model = BHM(n, 4, false, 0);
		
		for (i, m) in enumerate(mu)
			t[n, i] = binarySearch(model, 0.001, 0.2, 1e-5; U = 1., mu = m, psi_t = 1e-6)
		end
	end
end

# ╔═╡ b20aba71-2c4e-4b65-a5f7-43476799b705
begin
	theme(:lime)
	p = plot(xlims = [0., 0.2], title = "1D BHM", xlabel = "t/U", ylabel = "mu/U", framestyle = :box)
	
	for n in num_sites
		plot!(t[n, :], mu, lw = 1, lab = "$(n)-site CMFT")
	end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "8cdf92c1fe803e9cfdfbbd1ca1709dad9ef87351"
