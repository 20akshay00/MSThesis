...
begin
	order_param = [load("../data/cmft/1D_L=$(i)_nb=4_n=100.jld")["order_param"] for i in 1:5]
	t = load("../data/ED_data.jld")["t"]
	mu = load("../data/ED_data.jld")["mu"]
	exact = load("../data/ED_data.jld")["order_param"]
end;
...
begin
	theme(:dao)
	heatmap(t, mu, exact)
	for num_sites in 1:length(order_param)
		plot!(t[replace([findlast(<(1e-2), order_param[num_sites][i, :]) for i in 1:size(order_param[1])[2]], nothing => 1)], mu, label = "$(num_sites)-site CMFT", ls = :dash, lw = 2.5, c = num_sites, alpha = 0.8)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "7a04dbda0c753d31a0a346f9058efdb8564f4eed"
