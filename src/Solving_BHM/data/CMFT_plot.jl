...
begin
	...
	for num_sites in 1:4
		plot!(t[replace([findlast(<(1e-2), order_param[num_sites, i, :]) for i in 1:size(order_param)[2]], nothing => 1)], mu, label = "$(num_sites)-site CMFT", ls = :dash, lw = 2.5, c = num_sites, alpha = 0.8)
