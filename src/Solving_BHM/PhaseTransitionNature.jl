...
# ╔═╡ 8dce2710-1f86-11ed-0d83-3f57529e5212
using Plots, Optim, LinearAlgebra, LaTeXStrings

# ╔═╡ 67c338e5-bf92-4e4c-a8ab-5f54e8b5a830
begin
...
# ╔═╡ bcfa828f-f9fa-45cb-84d8-66a1cc82ca08
begin
	model = MeanField(10)
...
# ╔═╡ 47ac1720-9031-4739-90d1-dbfce691e362
let
	model = MeanField(10)
	
	z, size = 2, 500
	st, stp = 0.05 - 0.05, 0.05 + 0.07
	t = range(start = st, stop = stp, length = size)
	mu = 1.5
	U = 1

	order_param = zeros(size)
	num_particles = zeros(size)
	
	for k1 in 1:size
	    order_param[k1] = abs(get_order_parameter(model, t[k1], U, mu, z))
	    # num_particles[k1] = abs(get_num_particles(model, t[k1], U, mu, z))
	end

	p1 = plot(t, order_param, label = "")
	plot!(
	...
	p2 = plot(t[2:end], diff(order_param)./(t[2] - t[1]), label = "")
	plot!(
		ylabel = L"\frac{\partial}{\partial t}\langle a \rangle",
		xlabel = "t (hopping parameter)",
		framestyle = :box,
		title = "Order parameter derivative")

	# p2 = plot(t, num_particles, label = "")
	# plot!(
	# 	ylabel = L"\langle n \rangle",
	# 	xlabel = "t (hopping parameter)",
	# 	framestyle = :box,
	# 	title = "Average occupation number")
...
# ╔═╡ f1226e18-57ba-4f4d-95da-ba60e30154f4
begin
	theme(:lime)
	gr()
