...
# ╔═╡ cc0a3865-00dd-482f-9759-61161890782f
begin
...
# ╔═╡ 5550e55f-0540-4d45-924f-2e25a4524244
begin
	...
	function generate_number(mu, basis) 
	    ...
	    return spdiagm((sum(mu' .* basis, dims = 2) |> vec))
	end
end

# ╔═╡ 2a4c7cb7-fa20-488d-baad-e2d0aedaa695
begin
	...
	function num_stats(ground_state, basis, i)
...
# ╔═╡ d760307f-5145-4603-84bb-f56bbbece089
# ╠═╡ disabled = true
#=╠═╡
let
	N, L = 7, 7
	basis_set = generate_basis(N, L)
	Hkin = generate_kinetic(1, basis_set)
	Hint = generate_interaction(1, basis_set)

	occupations = []	
	ts = range(start = 0.01, stop = 1, length = 7)
	
	for t in ts
		Hn = generate_number(t * (-L÷2 : L÷2) .^ 2, basis_set)
		E, vecs = eigsolve(t * Hkin .+ Hint .+ Hn, 1, :SR)
		push!(occupations, [num_stats(vecs[1], basis_set, i)[1] for i in 1:L])
	end

	p = plot(framestyle = :box)

	for (i, occ) in enumerate(occupations)
		plot!(occ, lw = 2, st = [:line, :scatter], lab = ["t/U = $(ts[i])" ""], c = i, ms = 0, ls = :dot)
	end
...
# ╔═╡ b094ffc5-e80f-4534-86f1-f0c7119332d0
begin
	num_particles, num_sites = 8, 8
	basis_set = generate_basis(num_particles, num_sites);
	Hkin = generate_kinetic(1, basis_set);
	Hint = generate_interaction(1, basis_set);

	U = 0:0.5:20;
	fc = zeros(Float64, length(U));
	odlro = zeros(Float64, (length(U), num_sites÷2));
	nvars = zeros(Float64, length(U));
	
	for (i, u) in enumerate(U)
	    E, vecs = eigsolve(Hkin .+ u * Hint, 1, :SR)
	    ρ = SPDM(vecs[1], basis_set)
	
	    fc[i] = sum(ρ[1, :]) # largest eigenvalue
	    odlro[i, :] .= ρ[1, 1:(num_sites÷2)]
	    nvars[i] = num_stats(vecs[1], basis_set, num_sites ÷ 2)[3]
	end
end

# ╔═╡ 2ba7ab65-1589-4db6-9c44-21759a2517d8
begin
		p1 = plot(U, fc ./ num_particles, st = [:scatter, :line], labels = "", ylabel = L"Condensate fraction - $f_c$");
		plot!(U, fill(1/num_particles, length(U)), ls = :dash, label = "Limiting fraction", lw = 1.5, c = :black)
		plot!(framestyle = :box, title = "\nExact Diagonalization - $(num_sites) sites", titlefontsize = 11, ylims = [-0.1, 1.1], xformatter=_->"");
	
		p2 = plot(U, odlro, 
				st = :line, 
				labels = reshape([L"\rho_{1,%$(i)}" for i in 1:(num_sites÷2)], 1, num_sites÷2), 
				xlabel = L"$U/J$", 
				ylabel = "ODLRO - " * L"\langle a_i^{\dagger} a_j\rangle", c = reshape(1:(num_sites÷2), 1, num_sites÷2));
	
		plot!(U, odlro, st = :scatter, lab = "", c = reshape(1:(num_sites÷2), 1, num_sites÷2))
		plot!(U, zeros(length(U)), ls = :dash, label = "", c = :black, lw = 1.5)
		plot!(framestyle = :box, titlefontsize = 11, ylims = [-0.1, 1.1]);	

		p3 = plot(U, nvars, lw = 2, st = [:line, :scatter], label = "",
				ylims = [0, 1],
				ylabel = "Occ. Var. - " * L"\sqrt{\langle n_i \rangle ^2 - \langle n_i^2 \rangle}")
		plot!(framestyle = :box)
	
		plot(p1, p3, p2, size = (600, 900), left_margin=5Plots.mm, top_margin=6Plots.mm, layout = (3, 1), link = :x)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "9cc2b9254487f9c7bfb4a6c1464e5a222acdf209"
