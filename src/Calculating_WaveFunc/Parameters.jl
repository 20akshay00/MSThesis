...
begin
	...
	function tunneling(inds, bloch_energy, k)
		-1 * sum(
		1/length(k[i]) * real.(sum(bloch_energy[i] .* exp.(-1im * k[i] * (inds[1][i] - inds[2][i]))))
		for i in 1:3)
	end
...
function get_wannier(V0, kl, l_max, q)
		E = Array{Float64}(undef, length(q), 2 * l_max + 1)
		psi = Array{Float64}(undef, length(q), 2 * l_max + 1, 2 * l_max + 1);

	    for (j, k) in enumerate(q)
	        E[j, :], psi[j, :, :] = get_bloch_spectrum(V0, kl, k, l_max)
	    end

		bloch = [to_pos_basis(psi[ind, 1, :], kl, l_max) for ind in 1:size(psi)[1]]
...
# ╔═╡ 19479904-6fd5-40b9-803c-b4938b902960
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "9576f8142ebb8fec5204ef2613053da7d1024b8b"
